...
import DisplayProfileHeader from '../../DisplayProfile/DisplayProfileHeader'
...
class ProfilePage extends Component{
	constructor(props) {
		...
		this.state = { 	isLoaded: false,
						userData: [], // userData is the data of the user making the request
						userDataOfProfile: [],
						error: null,
						postData: [],
						userExtendedData: []
	...
	componentDidMount() {
		...
		fetch(ENV.url + `/post/get-post-objects-by-user-username-sort-by-likes/${this.props.params.username}`,
			{
				headers: {
					"method": "GET",
					"Accept": "application/json",
	...
	getUserExtendedObject = (user_pk) => {
		fetch(ENV.url + `/user-auth/get-user-extended-object-by-pk/${user_pk}`,
			{
				headers: {
					"method": "GET",
					"Accept": "application/json",
	...
	render() {
		...
		const { error, isLoaded, userData, postData, userExtendedData } = this.state
		...
		if (!isLoaded) {
			...
			//
		} else if (postData.length > 0 && isLoaded && userData.pk != []) {
			return (
				<div>
					<ThemeProvider theme={theme}>
